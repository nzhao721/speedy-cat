 ...
 *
 * Visible action items are horizontally scrollable via an [ActionMenuView] inside a
...
class ReviewerMenuView : LinearLayout {
    ...
    fun isEmpty() = frontMenu.size == 0

    fun findItem(id: Int): MenuItemImpl? = frontMenu.findItem(id) as? MenuItemImpl
    ...
    fun addActions(alwaysShow: List<ViewerAction>) {
    ...
    private fun setupMenus() {
        val menuItems = repository.getActionsByMenuDisplayTypes(MenuDisplayType.ALWAYS)
        addActions(menuItems.getValue(MenuDisplayType.ALWAYS))
