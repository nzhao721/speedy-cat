...
class SidebarTreeView(QTreeView):
    def __init__(self, browser: aqt.browser.Browser) -> None:
        ...
        # read-only sidebar: no inline renaming
    ...
    def tool(self, tool: SidebarTool) -> None:
        ...
        # read-only sidebar: dragging items to reorganise is disabled
    ...
    def show_context_menu(self, item: SidebarItem, index: QModelIndex) -> None:
        # The sidebar is a read-only filter/view, so the context menu only
        # exposes non-mutating actions: searching by the selection and
